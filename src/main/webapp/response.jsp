    ...
    <nav class="navbar navbar-default">
        <div class="container-fluid">
            ...
            <ul class="nav navbar-nav">
                ...
                <li class="active"><a href="response?action=response"><fmt:message key="menu.response"/></a></li>
                <c:choose>
                    <%--===========================================================================
            ...
            <ul class="nav navbar-nav navbar-right">
                ...
                <li><input type="image" src="https://img.icons8.com/color/25/000000/ukraine-circular.png" alt="<fmt:message key="ukrainian" />" onclick="location.href='response?action=response&sessionLocale=ua'"></li>

                <li><input type="image" src="images/english.png" alt="<fmt:message key="english" />" onclick="location.href='response?action=response&sessionLocale=en'"></li>
            </ul>
        </div>
    </nav>
</div>
    ...
    <div class="row">
        <div class="col-md-5">
            <div class="about-desc">
               <c:if test="${user == null}">
                    <h2><custom:Hello message = "Log in, to fill the review"/></h2>
                </c:if>
                ...
                <main class="m-3">
                    <div class="row col-md-6">
                        <table class="table table-striped table-bordered table-sm">
                            <tr>
                                <th><fmt:message key="register.name_and_surname"/></th>
                                <th><fmt:message key="response.review"/></th>
                                <th><fmt:message key="record_admin_list_date"/></th>
                                <th><fmt:message key="record_admin_list_master"/></th>
                            </tr>

                            <c:forEach var="allreview" items="${allreview}">
                                <tr>
                                    <td>${allreview.name}</td>
                                    <td>${allreview.text}</td>
                                    <td>${allreview.date}</td>
                                    <td>${allreview.name_of_master}</td>
                                </tr>
                            </c:forEach>
        ...
        <div class="col-md-7">
            <div class="about-image">
                <c:if test="${userRole.name == 'user'}">
                    <form method="post" action="controller?action=add_response">
                        <p><fmt:message key="response.your_response"/></p>
                        <input type="text" size="30" height="100px" name="response_text" placeholder="<fmt:message key="menu.response"/>" required>
                        <p><input type="submit" class="btn btn-warning" value="<fmt:message key="response.review.button"/>  " /></p>
                    </form>
                </c:if>
            </div>
        </div>
